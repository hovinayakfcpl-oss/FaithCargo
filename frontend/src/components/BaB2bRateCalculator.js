...
function B2BRateCalculator() {
  ...
  const [form, setForm] = useState({
    ...
    invoiceValue: "",
    codAmount: "",
    paymentMode: "", // Placeholder ke liye ise khali rakha hai
    insurance: false,
    appointment: false
  })
    ...
    <div className="main">
      <div className="layout">
        ...
        <div className="card">
          ...
          <div className="grid2">
            {/* ✅ Update: Placeholder for Payment Mode */}
          ...
          <div className="dim">
            ...
            {dimensions.map((d, i) => (
              <div key={i} className="dim-row">
                {/* ✅ Update: Added Placeholders for Clarity */}
                <input placeholder="Qty" value={d.qty} name="qty" onChange={(e) => handleDimChange(i, e)} />
                <input placeholder="L" value={d.length} name="length" onChange={(e) => handleDimChange(i, e)} />
                <input placeholder="W" value={d.width} name="width" onChange={(e) => handleDimChange(i, e)} />
                <input placeholder="H" value={d.height} name="height" onChange={(e) => handleDimChange(i, e)} />
            ...
            {/* ✅ Update: Added class for styling the button smaller */}
            <button className="add-btn-small" onClick={addBox}>+ Add Box</button>
            ...
            <div className="result-premium">
              ...
              {show && (
                <div className="rp-body">
                  ...
                  <div className="rp-row"><span>Freight (₹ {result.perKg}/Kg)</span><span>₹ {result.freight}</span></div>
                  ...
                  {result.is_oda && <div className="rp-row"><span>ODA</span><span>₹ {result.oda_charge}</span></div>}
                  {result.cod > 0 && <div className="rp-row"><span>COD</span><span>₹ 150</span></div>}
                  {result.handling > 0 && <div className="rp-row"><span>Handling</span><span>₹ 750</span></div>}
                  {form.insurance && <div className="rp-row"><span>Insurance</span><span>₹ {result.insurance}</span></div>}
                  {form.appointment && <div className="rp-row"><span>Appointment</span><span>₹ {result.appointment}</span></div>}
